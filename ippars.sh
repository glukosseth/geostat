...
sudo apt install python3-numpy python3-matplotlib libgeos-dev python3-geoip2 python3-mpltoolkits.basemap python3-pip -y >> /dev/null 2>&1
...
if [ -f "$bash_profile" ]; then
	...
	if [ ! $PORT ]; then
		read -e -p "Enter your RPC port : " PORT
		echo 'export PORT='${PORT} >> /dev/null 2>&1
	fi
	echo -e '\n\e[42mYour TOKEN is:' $TOKEN '\e[0m\n'
	echo -e '\n\e[42mYour RPC port is:' $PORT '\e[0m\n'
...
IPDATA=$HOME/geostat/iptest${DATA}.json
STATOUT=$HOME/geostat/OUTPUT${DATA}.txt
if [ -z "$IPDATA" ]; then
...
curl -sS http://localhost:${PORT}/net_info | jq -r '.result.peers[] | "\(.remote_ip)"' >> $IPDATA
...
echo "Please wait..."
python3 pygeoipmap.py -i "${IPDATA}"  --service m --db "$HOME/geostat/GeoLite2-City.mmdb"  --output "$HOME/geostat/map${DATA}.png" >> /dev/null 2>&1

echo -e '\n\e[42mSuccessfull draw map: \e[0m\n' $HOME/geostat/map${DATA}.png && sleep 1
...
#for line in `cat "/home/user/geostat/111.json"`
